...
# from https://github.com/jonnedtc/PupilDetector
# modified by René Ebertowski
...
class GradientIntersect:
    ...
    def locate(self, image, sigma=2, accuracy=1):
        ...
        # blur image
        blurred = cv.GaussianBlur(image, (5, 5), sigma)
...
class IsophoteCurvature:
    ...
    def locate(self, image):
        ...
        # normalize image
        image = cv.GaussianBlur(image, (5, 5), self.blur)
        ...
        center_map = cv.GaussianBlur(center_map, (5, 5), self.blur)
        blurred = cv.GaussianBlur(image, (5, 5), self.blur)
